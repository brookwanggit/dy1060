...
var MAC={
	...
	'Suggest': {
		'Show': function($id,$limit,$ajaxurl,$jumpurl){
			...
			$("#"+$id).autocomplete(SitePath+$ajaxurl,{
				width: 323,scrollHeight: 300,minChars: 1,matchSubset: 1,max: $limit,cacheLength: 10,multiple: true,matchContains: true,autoFill: false,dataType: "json",
				parse:function(obj) {
					if(obj.status){
						var parsed = [];
	...
	'Score': {
		...
		'Show':function($f,$tab,$id){
			var str = '';
			if($f==1){
				str = '<div style="padding:5px 10px;border:1px solid #CCC"><div>共 <strong style="font-size:14px;color:red" id="star_count"> 0 </strong> 个人评分， 平均分 <strong style="font-size:14px;color:red" id="star_pjf"> 0 </strong>， 总得分 <strong style="font-size:14px;color:red" id="star_all"> 0 </strong></div><div>';
				...
				str += '</ul>';
				str +='<span id="star_hover"></span><p><span id="star_shi">0</span><span id="star_ge">.0</span>分<span class="star_no">(已有<label id="star_count">0</label>人评分)</span></p></div>';
			}
			document.write(str);
