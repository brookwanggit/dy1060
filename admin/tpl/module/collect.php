...
elseif($method=='list')
{
    ...
    elseif($xt=='1'){
    	$url = $apiurl . '?ac=list&pg=' . $pg . '&rid='.$group . '&t=' . $type . '&wd=' . urlencode($wd).'&url='.$accesskey;
		...
		case 'sel':
			...
			switch($xt)
			{
				case '0': $url = "?action=cjsel&ids=".$ids;
					break;
				case '1': $url = "?ac=videolist&rid=".$group."&ids=".$ids.'&url='.$accesskey;
					break;
		...
		case 'day':
			switch($xt)
			{
				case '0': $url = "?action=cjday&rday=".$hour."&rpage=".$pg;
					break;
				case '1': $url = "?ac=videolist&rid=".$group."&h=".$hour."&pg=".$pg.'&url='.$accesskey;
					break;
		...
		case 'type':
			...
			switch($xt)
			{
				case '0': $url = "?action=cjtype&rpage=".$pg."&rtype=". $type;
					break;
				case '1': $url = "?ac=videolist&rid=".$group."&pg=" . $pg . "&t=" . $type.'&url='.$accesskey;
					break;
		...
		case 'all':
			switch($xt)
			{
				case '0': $url = "?action=cjall&rpage=".$pg;
					break;
				case '1': $url = "?ac=videolist&rid=".$group."&pg=". $pg.'&url='.$accesskey;
					break;
        ...
        elseif(strpos(','.$filter,$d_name)) { $des = '<font color="red">数据在过滤单中，跳过。</font>'; }
	        ...
	        if(!$row){
	        	...
	        	if($MAC['collect']['vod']['pic']==1){
					//if(strstr($d_pic,'npimg.php')) $d_pic = str_replace('/','',$d_pic);
